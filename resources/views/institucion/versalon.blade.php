            ...
            <div class="card card-primary dark">
              ...
              <div v-if="tipo!=0">
                ...
                <div v-if="tipo==3" class="row">
                  <div class="col-md-12 col-xs-12 col-sm-12">
                   <h1 class="titulos">Asignaturas del @{{salon.nombre}}</h1>
                    <table class="table">
                      <tr>
                        <th class="titulos">Nombre:</th>
                        <th class="titulos">
                          <form action="{{action('Institucion\InstitucionController@asignar_materia')}}" method="POST">
                              {{csrf_field()}}
                              <input type="hidden" name="idsalon" :value="salon.idsalon">
                              <input  type="submit" class="btn btn-info boton" name="operacion" value="Asignar/Eliminar Materias">

                          </form>
                        </th>
                        <th class="titulos">Maestro que la imparte:</th>
                        <th></th>
                      ...
                      <tr v-for="elemento in asignaturas">
                        ...
                        <td class="titulos">@{{elemento.nombremaestro}}</td>
                        <td>
                          <form action="{{action('Institucion\InstitucionController@asignar_maestro')}}" method="POST">
                              {{csrf_field()}}
                              <input type="hidden" name="idasignatura" :value="elemento.idasignatura">
                              <input type="hidden" name="idmaestro" :value="elemento.idmaestro">
                              <input type="hidden" name="idsalon" :value="salon.idsalon">
                              <input type="submit" class="btn btn-danger boton" name="operacion" value="Cambiar Maestro">
...
<!-- aQUI PODRAN PONER LAS OPCIONES DEL SIDEBAR SI NESESITAN MAS DE UNA YA SABEN UN FOREACH O VFOR PARA IR CREANDOLAS o solamente copian y pegan segun lo nesesiten --> 
  <li class="nav-item has-treeview menu-open">  
    <a  @click="estado_interaccion('No olvides elegir un elemento!',0)" href="#" class="nav-link">
      ...
      <p>
      Ver elementos del salon:
      <i class="right fas fa-angle-left"></i>
      </p>
    </a>
    <ul class="nav nav-treeview">
      <li class="nav-item">
          <a href="#"  @click="estado_interaccion('Perfecto, estos son los alumnos del {{$salon->nombre}}:',2)" class="nav-link">
      ...
      <li class="nav-item">
          <a href="#"  @click="estado_interaccion('Perfecto, estas son las asignaturas del {{$salon->nombre}}:',3)" class="nav-link">
